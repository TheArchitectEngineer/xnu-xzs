...
Phase D3 transitions from raw physical sector reads to full GUID Partition Table (GPT) discovery, parsing, and reciprocal Primary/Backup validation across all 55 partitions on the internal Samsung BJNB4R eMMC.

---

## 1. Phase D3-M1: Primary GPT Header Verification

### A. Objectives & Accomplishments
1. **Branch Policy Enforced**:
   - Created dedicated branch `xzs-d3-gpt` based on certified baseline `20cdf4a2c86f1b9eac7573479025ac618b505e84`.
   - Preserved `main`, `xzs-port`, `xzs-bringup`, and tag `xzs-d2-storage-complete`.
2. **True Host Oracle Independence Established**:
   - Independently captured and decoded `artifacts/oracles/mmcblk0_lba1.bin` via `scripts/host_gpt_oracle.py`.
   - Proved `HOST_HEADER_CRC32_STORED = 0xBFDF741D`, `CALCULATED = 0xBFDF741D`.
   - Kernel contains zero hard-coded oracle values; all validation is dynamic against physical bytes and live geometry.
3. **Hardware-Proven Primitive Refactored**:
   - Implemented `int xzs_emmc_read_sector_pio(uint32_t lba, uint8_t out[512])` with boundary check.
   - Physical read verified on hardware: `D3M1_LBA1_BYTE_FOR_BYTE_MATCH = yes` (SHA-256 `e4b891b42fd57eb352ffbe0aa9098d04fe85f88e3425dcba529064cce72f862a`).
4. **Dynamic Disk Bounds Validation**:
   - Decoded `LIVE_SEC_COUNT = 61071360` from fresh CMD8 `EXT_CSD`.
   - Verified `MyLBA == 1`, `AlternateLBA == 61071359`, `FirstUsableLBA == 34`, `LastUsableLBA == 61071326`.
5. **Partition Entry Geometry Validated**:
   - Verified `SizeOfPartitionEntry = 128` ($128 	imes 2^0$), `NumberOfPartitionEntries = 128`.
   - Multiplicative and additive overflow checks passed.
   - Geometry validated: $128 	imes 128 = 16384$ bytes, 32 sectors, LBA 2..33.
6. **Kernel CRC32 Validation & Immutability**:
   - Audited XNU `crc32()` in `src/xnu/bsd/libkern/crc32.c` (`<libkern/crc.h>`).
   - Copied 92 bytes into scratch buffer, zeroed `[0x10..0x13]`, computed CRC32.
   - Result: `GPT_HEADER_CRC32_STORED = 0xBFDF741D`, `CALCULATED = 0xBFDF741D`, `MATCH = yes`.
   - Verified raw buffer `g_xzs_gpt_lba1` remained unmutated (`GPT_LBA1_RAW_BUFFER_MUTATED = no`).
7. **Strict Phase Discipline**:
   - ZERO reads of LBA 2..33, ZERO partition enumeration.
   - `GPT_PARTITION_ARRAY_CRC32_VERIFIED = no`.
   - ZERO storage writes. Hard stop and warm reset to fastboot.

### B. Mandatory D3-M1 Summary
```text
...
Header CRC32:
stored=0xBFDF741D
calculated=0xBFDF741D

MyLBA=1
AlternateLBA=61071359

FirstUsableLBA=34
LastUsableLBA=61071326

PartitionEntryLBA=2
EntryCount=128
EntrySize=128

DiskGUID=98101b32-bbe2-4bf2-a06e-2bb33d000c20
...
## 2. Phase D3-M2A: Primary Partition Entry Array & CRC32

### A. Objectives & Accomplishments
1. **Dynamic Runtime Array Geometry**:
   - Geometry derived directly from verified Primary Header: `128 * 128 = 16,384` bytes across 32 consecutive physical sectors (`LBA 2..33`).
2. **Sequential Single-Block Transfers**:
   - Executed exactly 32 individual `CMD17` reads via PIO transport.
   - `READ_ATTEMPTS=32`, `READ_SUCCESS=32`, `DUPLICATE_READS=0`, `MISSING_READS=0`, `OUT_OF_RANGE_READS=0`.
   - ZERO console prints inside the live PIO read loop.
3. **Partition Entry Array CRC32 Verification**:
   - Computed `crc32(0, entries, 16384)` dynamically: `0x64EDE0F4`.
   - Compared against Primary Header `PartitionEntryArrayCRC32` (`0x64EDE0F4`): exact match.
4. **Buffer Immutability Proof**:
   - Evaluated `CRC_A` immediately after read and `CRC_B` after serialization; `CRC_A == CRC_B`.
5. **Byte-for-Byte Independent Oracle Equality**:
   - Reconstructed binary `artifacts/builds/d3m2a_primary_entries.bin` compared against TWRP oracle `artifacts/oracles/mmcblk0_gpt_primary_entries.bin`.
   - Both produce SHA-256 `8249f2bfa98ec675bf66b6342e29a0fbf6ab0ccb716c789a172f1f46b1828dd3` (`cmp` exit 0).
6. **Strict Hardware Boundary**:
   - 32 x `CMD17` single-block reads.
   - ZERO storage writes.
   - ZERO partition enumeration or decoding.

### B. Mandatory D3-M2A Summary
```text
Primary Entry Array:
LBA2..33
...
SHA256=
8249f2bfa98ec675bf66b6342e29a0fbf6ab0ccb716c789a172f1f46b1828dd3
...
## 3. Phase D3-M2B: Primary Partition Map Validation & Extent Auditing

### A. Objectives & Accomplishments
1. **Dynamic Slot Decoding**:
   - Decoded all 128 GPT partition-entry slots into static kernel storage (`sizeof=128` bytes each).
   - Classified entries: 55 used (`PartitionTypeGUID != 0`), 73 unused (`PartitionTypeGUID == 0`).
2. **Structural & Extent Integrity**:
   - All 55 used entries verified strictly within usable disk bounds `[34, 61071326]` and `< LIVE_SEC_COUNT` (`61071360`).
   - Arithmetic overflow checks: `sector_count <= UINT64_MAX / 512`, `start_lba <= end_lba` (0 overflows).
   - Pairwise extent overlap audit: all 1,485 entry pairs tested (`A.start <= B.end && B.start <= A.end`); 0 overlapping pairs detected.
3. **GUID & Canonical Name Verification**:
   - Unique GUID zero count: 0; duplicate Unique GUID count: 0 (all 55 unique GUIDs pairwise distinct).
   - Preserved raw 72-byte name hex and canonical UTF-16LE text representation.
4. **Terminology & A/B Classification**:
   - Partition name suffixes `_a` / `_b` were observed in metadata (`boot_a`/`boot_b`, `system_a`/`system_b`, etc.).
   - Explicitly classified: `GPT_NAME_PAIRED_A_B_ENTRIES_OBSERVED=yes`, `A_B_BOOT_SLOT_SEMANTICS=NOT_ESTABLISHED`. No functional Android A/B slot redundancy is claimed as proven.
5. **Host vs XNU Parity**:
   - Frozen host oracle `artifacts/oracles/gpt_primary_partition_map.json` (SHA-256 `13ee706f681f640b26b7a02350a27c7bc1e6ad4a7b95378f7c398b240b6b09e5`).
   - Silicon telemetry matched slot-by-slot on all 12 fields across all 55 used entries with 0 differences.

### B. Mandatory D3-M2B Summary
```text
GPT slots parsed=128

used=55
unused=73

Unique GUID zero count=0
duplicate Unique GUID count=0

invalid extent count=0
overlap pair count=0

HOST_VS_XNU_PRIMARY_PARTITION_MAP_MATCH=yes

GPT _a/_b name-paired entries observed
...
## 4. Phase D3-M3: Backup GPT Verification & Primary/Backup Cross-Validation

### A. Objectives & Accomplishments
1. **Independent Host Backup Oracle**:
   - Disk size retrieved independently via TWRP: `HOST_SEC_COUNT=61071360`, `HOST_LAST_PHYSICAL_LBA=61071359`.
   - Captured Backup GPT Header: `artifacts/oracles/mmcblk0_gpt_backup_header.bin` (512 bytes, SHA-256 `94434f5b0c7b8d547e2ef9ee6e3322ed1c82631310c172ab42a4878e4ec88aee`).
   - Header CRC32 verified: stored `0x03F02415`, calculated `0x03F02415`.
   - Derived Backup Entry Array geometry: `LBA 61071327..61071358` (32 sectors, 16,384 bytes).
   - Array immediately precedes Backup Header: `BACKUP_ARRAY_IMMEDIATELY_PRECEDES_HEADER=yes` (`61071358 + 1 == 61071359`).
   - Captured Backup Entry Array: `artifacts/oracles/mmcblk0_gpt_backup_entries.bin` (16,384 bytes, SHA-256 `8249f2bfa98ec675bf66b6342e29a0fbf6ab0ccb716c789a172f1f46b1828dd3`).
   - Host Primary vs Backup array comparison: `cmp` exit 0 (`HOST_PRIMARY_BACKUP_ARRAY_BYTE_MATCH=yes`).
   - Host Primary vs Backup map comparison: `artifacts/oracles/gpt_backup_partition_map.json` matched 55/55 used entries.
2. **Fresh Silicon Re-verification**:
   - SDC1 re-initialized; fresh Primary Header verified; fresh Primary Array verified; fresh Primary Map verified.
3. **Backup GPT Header Verification on Silicon**:
   - Target LBA resolved dynamically from `Primary.AlternateLBA = 61071359`.
   - Read via `CMD17`; verified signature `"EFI PART"`, size 92, reserved fields zero.
   - Header CRC32 verified: stored `0x03F02415`, calculated `0x03F02415`.
   - Reconstructed `artifacts/builds/d3m3_backup_header.bin` matches TWRP oracle (`cmp` exit 0).
4. **Reciprocal Header Semantics**:
   - `PRIMARY.MyLBA == BACKUP.AlternateLBA == 1`
   - `PRIMARY.AlternateLBA == BACKUP.MyLBA == 61071359`
   - Shared fields match: Revision (`0x00010000`), Usable Range (`[34, 61071326]`), DiskGUID (`98101b32-bbe2-4bf2-a06e-2bb33d000c20`), Entry Count (`128`), Entry Size (`128`), Entry Array CRC32 (`0x64EDE0F4`).
5. **Backup Entry Array Verification on Silicon**:
   - Read 32 consecutive physical sectors (`LBA 61071327..61071358`) via sequential `CMD17` PIO transfers.
   - `READ_ATTEMPTS=32`, `READ_SUCCESS=32`, `DUPLICATE_READS=0`, `MISSING_READS=0`, `OUT_OF_RANGE_READS=0`.
   - Array CRC32 verified: stored `0x64EDE0F4`, calculated `0x64EDE0F4`.
   - Reconstructed `artifacts/builds/d3m3_backup_entries.bin` matches TWRP oracle (`cmp` exit 0).
6. **Silicon In-Memory Equality & Map Cross-Check**:
   - Primary Array == Backup Array: 16,384 / 16,384 bytes compared in memory with 0 mismatches (`XNU_PRIMARY_BACKUP_ARRAY_BYTE_MATCH=yes`).
   - Decoded Backup Map into static storage using the unified parser: 55 used / 73 unused.
   - Primary Map == Backup Map: all 55 used partitions match across all 12 fields (`PRIMARY_BACKUP_PARTITION_MAP_MATCH=yes`).
7. **Offline Verifier Confirmation**:
   - `scripts/verify_d3m3_acceptance.py` passed all 69 telemetry keys.
   - `HOST_VS_XNU_BACKUP_HEADER_MATCH=yes`
   - `HOST_VS_XNU_BACKUP_ARRAY_MATCH=yes`
   - `HOST_VS_XNU_BACKUP_PARTITION_MAP_MATCH=yes`

### B. Mandatory D3-M3 Summary
```text
Backup Header LBA=61071359

Header CRC32:
stored=0x03F02415
calculated=0x03F02415

MyLBA=61071359
AlternateLBA=1

PartitionEntryLBA=61071327

EntryCount=128
EntrySize=128
EntryArrayCRC32=0x64EDE0F4

Backup Entry Array:
LBA61071327..61071358
32 sectors
16384 bytes
...
## 5. Phase D3 Final Milestone Seal

All hardware acceptance gates for Phase D3 have been certified on physical silicon:
...
### Git Checkpoint & Release Artifacts
```text
D3 development branch:
b2d20c9bb6411904a0f87ee0b088ba0a2a99e579

D3 integrated main:
d5e6f9b5f517fd73e6d10ad6065518b140b4cf1c
