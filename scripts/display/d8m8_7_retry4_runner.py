#!/usr/bin/env python3
"""D8-M8-7 Retry #4 Source-Faithful PingPong Timing + MDSS VSYNC Clock Runner.
Executes prerequisites M2..M6, M8-1..M8-3, enables MDSS VSYNC clock @ 19.2 MHz (XO),
executes M8-4..M8-6 (with source-faithful PP0 timing BIT19=1, BIT20=0, VCLKS_LINE=147=0x93,
and DSI_CMD_MDP_CTRL=0x00000008), verifies PREKICK_READY=YES, and then performs
exactly ONE controlled MDP command-mode kickoff (CTL_START).
Halts strictly after one attempt.
Saves all evidence to artifacts/hw/d8m8/m8-7-retry4/.
"""
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Retry #4 Source-Faithful PP Timing + VSYNC Clock Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7-retry4",
    ...
    print("=== D8-M8-7 RETRY #4 HARDWARE RUNNER START ===", flush=True)
    ...
    # =========================================================================
    # 7. EXECUTING M8-7 RETRY #4: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7 RETRY #4: CONTROLLED SINGLE KICKOFF ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 RETRY #4 KICKOFF", "display m8-kickoff\n", 25.0, required_substr="M8_7_RETRY4=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 RETRY #4 VERIFICATION ===", flush=True)
    if "M8_7_RETRY4=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 RETRY #4 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 RETRY #4 FAIL or TIMEOUT! Check artifacts for details. <<<", flush=True)
