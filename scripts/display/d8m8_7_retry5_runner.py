#!/usr/bin/env python3
"""D8-M8-7 Retry #5 Correct CTL0 -> INTF1 Routing + First-Play Interface Flush Runner.
Executes prerequisites M2..M6, M8-1..M8-3, enables MDSS VSYNC clock @ 19.2 MHz (XO),
executes M8-4..M8-6 (with CTL_TOP = 0x00020020 and CTL_FLUSH = 0x40020048),
...
Saves all evidence to artifacts/hw/d8m8/m8-7-retry5/.
"""
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Retry #5 Correct CTL0 Routing + First-Play Interface Flush Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7-retry5",
    ...
    print("=== D8-M8-7 RETRY #5 HARDWARE RUNNER START ===", flush=True)
    ...
    # =========================================================================
    # 7. EXECUTING M8-7 RETRY #5: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7 RETRY #5: CONTROLLED SINGLE KICKOFF ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 RETRY #5 KICKOFF", "display m8-kickoff\n", 25.0, required_substr="M8_7_RETRY5=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 RETRY #5 VERIFICATION ===", flush=True)
    if "M8_7_RETRY5=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 RETRY #5 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 RETRY #5 FAIL or TIMEOUT! Check artifacts for details. <<<", flush=True)
