#!/usr/bin/env python3
"""D8-M8-7 Retry #6 Integrated Primary-AI Audit + Real Command-Mode Panel-On + Single First Frame.
Executes prerequisites M2..M4, P1, real physical panel power-on & DCS sequence (m8-panel-prepare),
executes M8-1..M8-6 (with CTL_TOP = 0x00020020 and CTL_FLUSH = 0x40020048),
...
Saves all evidence to artifacts/hw/d8m8/m8-7-retry6/.
"""
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Retry #6 Real Panel-On + First Frame Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7-retry6",
    ...
    print("=== D8-M8-7 RETRY #6 HARDWARE RUNNER START ===", flush=True)

    if not args.no_boot:
        print("=== STEP 0: FRESH BOOT VIA FASTBOOT ===", flush=True)
        # Check if device is in fastboot; if in XNU, reboot to fastboot first
    ...
    # =========================================================================
    # 7. EXECUTING M8-7 RETRY #6: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7 RETRY #6: CONTROLLED SINGLE KICKOFF ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 RETRY #6 KICKOFF", "display m8-kickoff\n", 25.0, required_substr="M8_7_RETRY6=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 RETRY #6 VERIFICATION ===", flush=True)
    if "M8_7_RETRY6=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 RETRY #6 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 RETRY #6 FAIL or TIMEOUT! Check artifacts for details. <<<", flush=True)
