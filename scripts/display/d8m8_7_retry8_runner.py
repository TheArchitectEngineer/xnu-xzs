#!/usr/bin/env python3
"""D8-M8-7 Retry #8: Enable PP0 Command-Mode Trigger Engine (PP0_TEAR_CHECK_EN = 1) + Single First Frame.
Executes prerequisites M2..M4, P1, real physical panel power-on & DCS sequence (m8-panel-prepare),
executes M8-1..M8-6 (with PP0_TEAR_CHECK_EN = 1, CTL_TOP = 0x00020020, CTL_FLUSH = 0x00020048),
verifies PREKICK_READY=YES and PANEL_READY=yes, and then performs exactly ONE controlled
MDP command-mode kickoff (CTL_START).
Captures high-frequency diagnostics during the first 20ms and poll loop.
Halts strictly after one attempt.
Saves all evidence to artifacts/hw/d8m8/m8-7-retry8/.
"""
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Retry #8 Enable PP0 Trigger Engine Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7-retry8",
    ...
    print("=== D8-M8-7 RETRY #8 HARDWARE RUNNER START ===", flush=True)
    ...
    print("\n=== EXECUTING STAGE M8-4: PP0 & DSI MDP STREAM PROGRAMMING + MDSS VSYNC CLOCK ===", flush=True)
    ...
    # =========================================================================
    # 7. EXECUTING M8-7 RETRY #8: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7 RETRY #8: CONTROLLED SINGLE KICKOFF ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 RETRY #8 KICKOFF", "display m8-kickoff\n", 25.0, required_substr="M8_7_RETRY8=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 RETRY #8 VERIFICATION ===", flush=True)
    if "M8_7_RETRY8=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 RETRY #8 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 RETRY #8 FAIL or TIMEOUT! Check artifacts for details. <<<", flush=True)
