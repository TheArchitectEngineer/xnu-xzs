#!/usr/bin/env python3
"""Scripted D8-P2 execution session over USB bulk console.
Executes M2 clocks, M3 PLL & PHY bringup, M4 DSI host enable,
P1 GPIO configuration, P2 SPMI read diagnostics, P2 dry-run,
P2 Stage 1 configuration, P2 Stage 2 staged enable and mandatory safe disable.
"""
...
def collect(dev, seconds=1.0):
    ...
    while time.time() < end:
        ...
        if chunk:
            ...
            quiet += 1
            if buf and b"xzs#" in buf and quiet >= 4:
                break
            time.sleep(0.05)
    return buf
...
def main():
    parser = argparse.ArgumentParser(description="D8-P2 Scripted Execution Session")
    parser.add_argument("--mode", choices=["dryrun", "config", "run"], default="run",
                        help="Execution mode: dryrun, config (config-only), or run (full staged enable/disable)")
    parser.add_argument("--log-dir", default="artifacts/hw/d8p2/run1",
    ...
    print(f"=== D8-P2 SCRIPTED SESSION START (MODE={args.mode}) ===", flush=True)
    ...
    if "RESULT=PASS_FULL_M3" not in m3_out:
        print("!!! M3 LOWER LAYER FAILED. Halting before P2.", flush=True)
    ...
    if "RESULT=PASS_MODE2" not in m4_out:
        print("!!! M4 DSI HOST FAILED. Halting before P2.", flush=True)
        log_file.write_text("".join(full_log))
        sys.exit(1)
    ...
    if "RESULT=PASS_P1" not in p1_out:
        print("!!! D8-P1 HARDWARE EXECUTION FAILED. Halting before P2.", flush=True)
        log_file.write_text("".join(full_log))
        sys.exit(1)
    print(">>> D8-P1 TLMM GPIOS CONFIGURED & VERIFIED SAFE! <<<")

    # 6. Read-Only SPMI & PMIC Diagnostics
    print("\n=== STEP 1: READ-ONLY SPMI & PMIC DIAGNOSTICS ===", flush=True)
    run_step("SPMI ARBITER STATUS", "display spmi status\n", 3.0)
    run_step("PMIC LAB STATUS (READ-ONLY)", "display pmic lab status\n", 3.0)
    run_step("PMIC IBB STATUS (READ-ONLY)", "display pmic ibb status\n", 3.0)

    # 7. D8-P2 Dry-Run Verification
    print("\n=== STEP 2: D8-P2 DRY-RUN ===", flush=True)
    dry_out = run_step("DRY-RUN D8-P2", "display p2-dryrun\n", 5.0)
    if "RESULT=PASS_DRYRUN" not in dry_out:
        print("!!! D8-P2 DRY-RUN FAILED. Halting.", flush=True)
        log_file.write_text("".join(full_log))
        sys.exit(1)
    print(">>> D8-P2 DRY-RUN PASSED 100%! <<<")
    ...
    # 8. Stage 1: Configuration Without Enable
    print("\n=== STEP 3: STAGE 1 CONFIGURATION WITHOUT ENABLE ===", flush=True)
    cfg_out = run_step("RUN P2 CONFIG ONLY", "display p2-config\n", 5.0)
    if "RESULT=PASS_CONFIG_ONLY" not in cfg_out:
        print("!!! D8-P2 CONFIGURATION FAILED. Halting.", flush=True)
        log_file.write_text("".join(full_log))
        sys.exit(1)
    print(">>> D8-P2 CONFIGURATION WITHOUT ENABLE PASSED! <<<")

    if args.mode == "config":
        print(">>> Config-only complete. Exiting per --mode config. <<<")
        log_file.write_text("".join(full_log))
        return

    # 9. Stage 2: Staged Enable, Verification, and Mandatory Safe Disable
    print("\n=== STEP 4: STAGE 2 FULL HARDWARE ENABLE & SAFE DISABLE ===", flush=True)
    p2_out = run_step("RUN P2 FULL", "display p2-run\n", 15.0)
    if "RESULT=PASS_FULL_P2" not in p2_out:
        print("!!! D8-P2 FULL HARDWARE RUN FAILED. Halting.", flush=True)
        log_file.write_text("".join(full_log))
        sys.exit(1)
    print(">>> D8-P2 FULL HARDWARE ENABLE & SAFE DISABLE PASSED! <<<")

    # 10. Post-P2 Status Verification
    print("\n=== STEP 5: POST-P2 STATUS VERIFICATION ===", flush=True)
    run_step("POST-P2 LAB STATUS", "display pmic lab status\n", 3.0)
    run_step("POST-P2 IBB STATUS", "display pmic ibb status\n", 3.0)
    run_step("POST-P2 GPIO STATUS", "display gpio status\n", 3.0)
    run_step("POST-P2 DSI HOST STATUS", "display m4-status\n", 3.0)

    # 11. Final Health & Register Dump
    ...
    with open(log_dir / "xnu_post_p2_registers.json", "w") as f:
