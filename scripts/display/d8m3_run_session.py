#!/usr/bin/env python3
"""Scripted D8-M3 execution session over USB bulk console.
Executes display power bringup, register snapshot, dry-run, real PLL programming,
and full PHY bring-up on Sony Xperia XZs (MSM8996 v3.0).
"""
...
def parse_regs_dump(output_text):
    """Extract 0x... = 0x... from console output."""
    regs = {}
...
def main():
    parser = argparse.ArgumentParser(description="D8-M3 Scripted Execution Session")
    parser.add_argument("--mode", choices=["dryrun", "pll", "full"], default="pll",
                        help="Execution mode: dryrun, pll (Stage A only), or full (Stage A+B)")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m3/run1",
    ...
    print(f"=== D8-M3 SCRIPTED SESSION START (MODE={args.mode}) ===", flush=True)
    ...
    print("\n=== POWERING DISPLAY GDSC & CORE CLOCKS ===", flush=True)
    ...
    # 3. Read Baseline Register Snapshot
    print("\n=== STEP 1: PRE-PROGRAMMING REGISTER SNAPSHOT ===", flush=True)
    base_regs_out = run_step("SNAPSHOT BASELINE REGS", "display regs\n", 2.0)
    base_regs = parse_regs_dump(base_regs_out)
    with open(log_dir / "xnu_pre_m3_registers.txt", "w") as f:
        for addr in sorted(base_regs.keys()):
            f.write(f"0x{addr:08x}: 0x{base_regs[addr]:08x}\n")
    with open(log_dir / "xnu_pre_m3_registers.json", "w") as f:
        json.dump({f"0x{addr:08x}": f"0x{val:08x}" for addr, val in sorted(base_regs.items())}, f, indent=2)
    print(f"Captured {len(base_regs)} pre-M3 registers.")

    # 4. Dry-Run Verification (Mode 0)
    print("\n=== STEP 2: DRY-RUN VERIFICATION (MODE 0) ===", flush=True)
    dry_out = run_step("DRY-RUN STATE MACHINE", "display m3-dryrun\n", 2.5)
    if "RESULT=PASS_DRYRUN" not in dry_out:
        print("!!! DRY-RUN FAILED. Halting before any hardware writes.", flush=True)
        log_file.write_text("".join(full_log))
        sys.exit(1)
    print(">>> DRY-RUN PASSED 100%! <<<")
    ...
    if args.mode == "pll":
        ...
        if "RESULT=PASS_PLL_STAGE" not in pll_out:
            ...
            log_file.write_text("".join(full_log))
            sys.exit(1)
        print(">>> PLL HARDWARE LOCK & CLOCKS PASS CONFIRMED! <<<")
        run_step("DUMP PLL STATUS POST-LOCK", "dsi pll\n", 1.5)
        ...
        if "RESULT=PASS_FULL_M3" not in full_out:
            ...
            run_step("DUMP PLL STATUS", "dsi pll\n", 1.5)
            run_step("DUMP PHY STATUS", "dsi phy\n", 1.5)
            log_file.write_text("".join(full_log))
            sys.exit(1)
        print(">>> FULL M3 PROGRAMMING PASSED (PLL LOCKED + CLOCKS + PHY READY)! <<<")
        run_step("DUMP PLL STATUS POST-M3", "dsi pll\n", 1.5)
        run_step("DUMP PHY STATUS POST-M3", "dsi phy\n", 1.5)

    # 6. Post-Programming Register Snapshot
    print("\n=== STEP 4: POST-PROGRAMMING REGISTER SNAPSHOT ===", flush=True)
    post_regs_out = run_step("SNAPSHOT POST-M3 REGS", "display regs\n", 2.0)
    post_regs = parse_regs_dump(post_regs_out)
    with open(log_dir / "xnu_post_m3_registers.txt", "w") as f:
        for addr in sorted(post_regs.keys()):
            f.write(f"0x{addr:08x}: 0x{post_regs[addr]:08x}\n")
    post_json_path = log_dir / "xnu_post_m3_registers.json"
    with open(post_json_path, "w") as f:
        json.dump({f"0x{addr:08x}": f"0x{val:08x}" for addr, val in sorted(post_regs.items())}, f, indent=2)
    print(f"Captured {len(post_regs)} post-M3 registers.")

    # 7. Post-M3 Status & System Health
    ...
    # 8. Automated Register Diff against Linux Golden
