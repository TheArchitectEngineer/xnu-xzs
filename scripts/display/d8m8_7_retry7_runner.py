#!/usr/bin/env python3
"""D8-M8-7 Retry #7 Integrated Primary-AI Audit + Correct Command-Mode Flush (0x00020048) + Single First Frame.
Executes prerequisites M2..M4, P1, real physical panel power-on & DCS sequence (m8-panel-prepare),
executes M8-1..M8-6 (with CTL_TOP = 0x00020020 and CTL_FLUSH = 0x00020048),
...
Saves all evidence to artifacts/hw/d8m8/m8-7-retry7/.
"""
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Retry #7 Real Panel-On + First Frame Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7-retry7",
    ...
    print("=== D8-M8-7 RETRY #7 HARDWARE RUNNER START ===", flush=True)
    ...
    # =========================================================================
    # 7. EXECUTING M8-7 RETRY #7: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7 RETRY #7: CONTROLLED SINGLE KICKOFF ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 RETRY #7 KICKOFF", "display m8-kickoff\n", 25.0, required_substr="M8_7_RETRY7=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 RETRY #7 VERIFICATION ===", flush=True)
    if "M8_7_RETRY7=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 RETRY #7 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 RETRY #7 FAIL or TIMEOUT! Check artifacts for details. <<<", flush=True)
