#!/usr/bin/env python3
"""D8-M8-7 Retry #9: Exact Keyaki Tearcheck Geometry + External TE Synchronization.
Executes prerequisites M2..M4, P1, real physical panel power-on & DCS sequence (m8-panel-prepare),
executes M8-1..M8-6 (with exact Keyaki geometry: PP0_SYNC_CFG_VSYNC = 0x00180093,
PP0_SYNC_WRCOUNT = 0x00000785, PP0_START_POS = 0x00000780, DSI_TRIG_CTRL = 0x80000004),
verifies PREKICK_READY=YES, PANEL_READY=yes, and PHYSICAL_TE_PROVEN=yes, and then performs
exactly ONE controlled MDP command-mode kickoff (CTL_START).
Captures high-frequency diagnostics during the first 20ms and poll loop.
Halts strictly after one attempt.
Saves all evidence to artifacts/hw/d8m8/m8-7-retry9/.
"""
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Retry #9 Exact Keyaki Tearcheck Geometry & External TE Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7-retry9",
    ...
    print("=== D8-M8-7 RETRY #9 HARDWARE RUNNER START ===", flush=True)
    ...
    print("\n=== EXECUTING STAGE M8-4: PP0 & DSI MDP STREAM PROGRAMMING (RETRY #9 EXACT GEOMETRY) ===", flush=True)
    ...
    assert "PP_SYNC_CONFIG_VSYNC=0x00180093" in prekick_out, "PP_SYNC_CONFIG_VSYNC must be 0x00180093"
    ...
    assert "PHYSICAL_TE_PROVEN=yes" in prekick_out, "PHYSICAL_TE_PROVEN must be yes"
    assert "PREKICK_READY=YES" in prekick_out, "PREKICK_READY must be YES"

    # =========================================================================
    # 7. EXECUTING M8-7 RETRY #9: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7 RETRY #9: CONTROLLED SINGLE KICKOFF ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 RETRY #9 KICKOFF", "display m8-kickoff\n", 25.0, required_substr="M8_7_RETRY9=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 RETRY #9 VERIFICATION ===", flush=True)
    if "M8_7_RETRY9=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 RETRY #9 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 RETRY #9 FAIL or TIMEOUT! Check artifacts for details. <<<", flush=True)
