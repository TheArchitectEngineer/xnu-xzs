#!/usr/bin/env python3
"""D8-M8-7 Controlled Single Kickoff Hardware Runner.
Executes prerequisites M2..M6, M8-1..M8-6, verifies PREKICK_READY=YES,
and then performs exactly ONE controlled MDP command-mode kickoff (CTL_START).
...
def main():
    parser = argparse.ArgumentParser(description="D8-M8-7 Controlled Kickoff Runner")
    parser.add_argument("--log-dir", default="artifacts/hw/d8m8/m8-7",
    ...
    print("=== D8-M8-7 HARDWARE RUNNER START ===", flush=True)

    if not args.no_boot:
        print("=== STEP 0: BOOTING VIA FASTBOOT ===", flush=True)
    ...
    # =========================================================================
    # 7. EXECUTING M8-7: CONTROLLED SINGLE KICKOFF
    # =========================================================================
    print("\n=======================================================", flush=True)
    print("=== EXECUTING M8-7: CONTROLLED SINGLE KICKOFF       ===", flush=True)
    print("=======================================================", flush=True)

    kickoff_out = run_step("M8-7 KICKOFF", "display m8-kickoff\n", 15.0, required_substr="M8_7=")
    ...
    # Final Verification
    print("\n=== FINAL M8-7 VERIFICATION ===", flush=True)
    if "M8_7=PASS" in kickoff_out:
        print(">>> RESULT: M8-7 PASS! First MDP command-mode frame scanned out! <<<", flush=True)
    else:
        print(">>> RESULT: M8-7 FAIL or BLOCKED! Check artifacts for details. <<<", flush=True)
