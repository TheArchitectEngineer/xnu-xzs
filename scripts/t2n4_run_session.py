#!/usr/bin/env python3
"""
Hardware execution session for D7-T2N-4: NATIVE PAGER DATA INTEGRITY & EOF VALIDATION.
...
def main():
    # Find commit hash for directory naming
    ...
    log_dir = Path(f"artifacts/hw/t2n4-{short_commit}")
    ...
    # Step 1: Shell health baseline (Section 24)
    ...
    # Step 4: Hardware validation — EOF page (Section 26)
    run_step("STEP 4: EOF PAGE VALIDATION (offset=16384, size=16384)", "xzsfs pagecheck /bin/hello 16384 16384\n", 3.0)

    # Step 5: Execute /bin/hello once (Section 27)
    print("\n--- STEP 5: EXECUTE /bin/hello (ONCE) ---", flush=True)
    hello_out = send_cmd(dev, "/bin/hello\n", wait_sec=8.0, wait_for_prompt=True)
    full_log.append(f"\n# STEP 5: EXECUTE /bin/hello\n> /bin/hello\n{hello_out}")

    # Step 6: Post-exec health check (Section 27, 28)
    print("\n--- STEP 6: POST-EXEC HEALTH CHECK ---", flush=True)
    try:
        post_out = send_cmd(dev, "pwd\n", wait_sec=3.0, wait_for_prompt=True)
        full_log.append(f"\n# STEP 6: POST-EXEC PWD\n> pwd\n{post_out}")
    ...
    print("=== D7-T2N-4 HARDWARE SESSION COMPLETE ===", flush=True)
