#ifndef _BOOTSHIM_UART_H_
#define _BOOTSHIM_UART_H_
...
#define XZS_PSTORE_DMESG_BASE    0x00000000a7f00000UL
#define XZS_PSTORE_DMESG_SIZE    (128 * 1024)
#define XZS_PSTORE_CONSOLE_BASE  0x00000000a7f80000UL
#define XZS_PSTORE_CONSOLE_SIZE  (256 * 1024)
#define XZS_PSTORE_SIG           0x43474244UL /* "DBGC" */

struct pstore_ram_buffer {
    uint32_t sig;
    uint32_t start;
    uint32_t size;
    char     data[];
...
struct xzs_debug_log {
    ...
    uint32_t write_offset;    /* 0x08: text log offset */
    uint32_t boot_count;      /* 0x0c: boot counter */
    uint32_t exception_count; /* 0x10: exception counter */
    uint32_t last_stage;      /* 0x14: enum xzs_stage */
    uint64_t last_elr;        /* 0x18: ELR_EL1 */
    uint64_t last_esr;        /* 0x20: ESR_EL1 */
    uint64_t last_far;        /* 0x28: FAR_EL1 */
    uint64_t last_spsr;       /* 0x30: SPSR_EL1 */
    uint8_t  reserved[8];     /* 0x38: padding to 64-byte aligned header */
    char data[XZS_DEBUG_LOG_SIZE]; /* 0x40: total size = 65536 = 64KB */
};

extern volatile struct xzs_debug_log *g_dlog;
