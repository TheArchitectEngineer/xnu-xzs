...
void xzs_early_puts(const char *s);
extern "C" void xzs_watchdog_pet(void);
    ...
    dev_t * root, u_int32_t * oflags )
{
	...
	// Bypassed on Xperia XZs: avoid 30s blocking sleep that deadlocks without timer IRQs
	xzs_early_puts("[XZS-BOOT] [D50-IOKIT] IOFindBSDRoot ENTER\n");
	...
	xzs_early_puts("[XZS-BOOT] [D50-I0] alloc buffer ENTER\n");
	str = OSDataAllocation<char>( kMaxPathBuf + kMaxBootVar, OSAllocateMemory );
	if (!str) {
		xzs_early_puts("[XZS-BOOT] [D50-I0] alloc buffer FAILED: kIOReturnNoMemory\n");
		return kIOReturnNoMemory;
	}
	xzs_early_puts("[XZS-BOOT] [D50-I0a] alloc buffer RETURN\n");
	...
	xzs_early_puts("[XZS-BOOT] [D50-I1] fromPath /chosen ENTER\n");
	if ((regEntry = IORegistryEntry::fromPath( "/chosen", gIODTPlane ))) {
		xzs_early_puts("[XZS-BOOT] [D50-I1a] fromPath /chosen RETURN found\n");
		do {
		...
		xzs_early_puts("[XZS-BOOT] [D50-I1a] fromPath /chosen RETURN not found\n");
	}
...
//
	...
	if (!didRam) {                                                                                           /* Have we already build this ram disk? */
		...
		xzs_early_puts("[XZS-BOOT] [D50-I2] fromPath /chosen/memory-map ENTER\n");
		if ((regEntry = IORegistryEntry::fromPath( "/chosen/memory-map", gIODTPlane ))) {        /* Find the map node */
			xzs_early_puts("[XZS-BOOT] [D50-I2a] fromPath /chosen/memory-map RETURN found\n");
...
#define MAX_PHYS_RAM    (((uint64_t)UINT_MAX) << 12)
			...
			xzs_early_puts("[XZS-BOOT] [D50-I2a] fromPath /chosen/memory-map RETURN not found\n");
		}
	}
...
//
	...
	if (!matching) {
		...
		xzs_early_puts("[XZS-BOOT] [D50-I3] serviceMatching(IOMedia) ENTER\n");
		...
		xzs_early_puts("[XZS-BOOT] [D50-I3a] serviceMatching(IOMedia) RETURN\n");
	}

	if (gIOKitDebug & kIOWaitQuietBeforeRoot) {
		xzs_early_puts("[XZS-BOOT] [D50-I4] waitQuiet ENTER\n");
		IOLog( "Waiting for matching to complete\n" );
		IOService::getPlatform()->waitQuiet();
		xzs_early_puts("[XZS-BOOT] [D50-I4a] waitQuiet RETURN\n");
	} else {
		xzs_early_puts("[XZS-BOOT] [D50-I4] waitQuiet SKIPPED (not set in gIOKitDebug)\n");
	}
	...
	if (needNetworkKexts
		...
		xzs_early_puts("[XZS-BOOT] [D50-I6] startDeferredMatches ENTER\n");
		IOService::startDeferredMatches();
		xzs_early_puts("[XZS-BOOT] [D50-I6a] startDeferredMatches RETURN\n");
	} else {
		xzs_early_puts("[XZS-BOOT] [D50-I6] startDeferredMatches SKIPPED\n");
	}

#ifndef CONFIG_XZS_BRINGUP
#define CONFIG_XZS_BRINGUP 1
#endif

#if CONFIG_XZS_BRINGUP
	/*
	 * [XZS-SELFTEST] Bounded root-device wait:
	 * Prove canonical IOKit matching path reached, and verify that without physical
	 * UFS storage driver, no matching IOMedia appears.
	 * Bounded wait: 100 steps of ~10ms = ~1.0s.
	 * WATCHDOG DISCIPLINE: Do NOT pet watchdog inside the loop or per-iteration.
	 * Only pet after the entire stage returns.
	 */
	xzs_early_puts("[XZS-BOOT] [D50-I7] canonical root-service wait ENTER\n");
	xzs_early_puts("[XZS-BOOT] [XZS-SELFTEST] bounded root-device wait (timeout=1.0s)...\n");
	for (uint32_t poll_step = 0; poll_step < 100; poll_step++) {
		service = IOService::copyMatchingService( matching );
		if (service) {
			service->release(); // match waitForService unretained semantics
			break;
		}
		/* ~10ms delay per step (192,000 ticks @ 19.2MHz) */
		uint64_t s, c;
	...
	if (!service) {
		xzs_early_puts("[XZS-BOOT] [D50-I8] no matching physical root service\n");
		xzs_early_puts("[XZS-BOOT] [XZS-SELFTEST] root-device wait timed out: no matching physical IOMedia\n");
		IOLog( "IOFindBSDRoot: no root device matched after timeout, failing gracefully\n" );
	}
	xzs_watchdog_pet();
#else
	xzs_early_puts("[XZS-BOOT] [D50-I7] canonical root-service wait ENTER\n");
	service = IOService::waitForMatchingService( matching );
	xzs_watchdog_pet();
#endif
	...
	if (!service) {
		xzs_early_puts("[XZS-BOOT] [D50-IOKIT] IOFindBSDRoot: returning kIOReturnNotFound (0xe00002bc)\n");
...
extern "C" void xzs_early_puts(const char *s);

static void
IOResolveCoreFilePath()
{
	xzs_early_puts("    [IOResolveCoreFilePath] ENTER\n");
	DTEntry node;
	const char *value = NULL;
	unsigned int size = 0;

	if (kSuccess != SecureDTLookupEntry(NULL, "/product", &node)) {
		xzs_early_puts("    [IOResolveCoreFilePath] /product not found, RETURN\n");
		return;
	}
	if (kSuccess != SecureDTGetProperty(node, "kernel-core-dump-location", (void const **) &value, &size)) {
		xzs_early_puts("    [IOResolveCoreFilePath] no prop, RETURN\n");
		return;
	}
	if (size == 0) {
		xzs_early_puts("    [IOResolveCoreFilePath] size 0, RETURN\n");
		return;
	}
	...
	if (strlcmp(value, "preboot", size) == 0) {
		...
		xzs_early_puts("    [IOResolveCoreFilePath] non-default, RETURN\n");
		return;
	}

	IOLog("corefile path selection in device-tree was set to: %s (value: %s)\n", kdp_corefile_path, value);
	xzs_early_puts("    [IOResolveCoreFilePath] RETURN\n");
}
