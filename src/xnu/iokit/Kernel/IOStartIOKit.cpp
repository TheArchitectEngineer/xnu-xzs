...
IOKitInitializeTime( void )
{
	 ...
	 * On Xperia XZs / MSM8996 early bootstrap, no IORTC service is registered yet.
	 * The canonical 30s timeout causes an APCS hardware watchdog bite (10s bite timer).
	 * We perform a non-blocking check (timeout=0).
	 * Long-term solution: implement Qualcomm PMIC RTC provider for IORTC.
	 */
#if defined(CONFIG_XZS_BRINGUP)
	t.tv_sec = 0;
	t.tv_nsec = 0;
#else
	t.tv_sec = 30;
