...
PE_init_iokit(void)
{
	...
	xzs_early_puts("[XNU-XZS] PE_init_iokit ENTER\n");
	...
	/* COMPATIBILITY / XZS_HEADLESS_PLATFORM:
	 * If no valid framebuffer is present (v_baseAddr == 0, v_width == 0, or v_height == 0)
	 * or v_display is 0 (headless serial console), disable progress display and skip
	 * the entire progress-display positioning / rendering path.
	 */
	if (PE_state.video.v_display == 0 || PE_state.video.v_width == 0 || PE_state.video.v_height == 0 || PE_state.video.v_baseAddr == 0) {
		xzs_early_puts("[XNU-XZS] HEADLESS video path selected\n");
		show_progress = FALSE;
	}
	...
	/*
	 * COMPATIBILITY / XZS_HEADLESS_PLATFORM:
	 * In standard macOS, com.apple.kec.corecrypto is loaded during InitIOKit()
	 * from the prelinked kernelcache, registering the kernel PRNG via register_and_init_prng().
	 * On standalone XZS (booted via fastboot boot without proprietary Apple kexts),
	 * initialize and register the in-kernel NIST HMAC-SHA256 PRNG provider here before EXIT.
	 */
	extern void xzs_prng_bootstrap(void);
	xzs_prng_bootstrap();

	xzs_early_puts("[XNU-XZS] PE_init_iokit EXIT\n");
}
...
PE_init_platform(boolean_t vm_initialized, void *args)
{
	...
	if (!vm_initialized) {
		 ...
		 * can be used..
		 */
		xzs_early_puts("  [PE] 1: SecureDTInit starting...\n");
		SecureDTInit(PE_state.deviceTreeHead, PE_state.deviceTreeSize);
		xzs_early_puts("  [PE] 2: SecureDTInit done, pe_identify_machine starting...\n");
		pe_identify_machine(boot_args_ptr);
		xzs_early_puts("  [PE] 3: pe_identify_machine done\n");
	} else {
...
#endif /* not XNU_TARGET_OS_XR */
		xzs_early_puts("  [PE] 4: pe_init_debug starting...\n");
		pe_init_debug();
		xzs_early_puts("  [PE] 5: pe_init_debug done\n");
	}
}
