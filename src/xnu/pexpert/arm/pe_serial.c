...
#define DRAM_LOG_SIZE  65472

struct xzs_debug_log {
	uint32_t magic;           /* 0x00: "XZSD" */
	...
	char data[DRAM_LOG_SIZE]; /* 0x40 */
};

static volatile struct xzs_debug_log *g_dram_log = NULL;

static inline void msm_uart_write(uint32_t offset, uint32_t val)
{
	*(volatile uint32_t *)(msm_uart_base + offset) = val;
}
...
static unsigned int
msm_uart_transmit_ready(void)
{
	if (!msm_uart_base) return 1;
	return (msm_uart_read(MSM_UART_SR) & MSM_UART_SR_TX_EMPTY) != 0;
}

static void
msm_uart_transmit_data(uint8_t c)
{
	/* 1. Record character into preserved DRAM log buffer */
	if (g_dram_log && g_dram_log->magic == DRAM_LOG_MAGIC) {
		uint32_t idx = g_dram_log->write_offset;
		if (idx < DRAM_LOG_SIZE - 1) {
			g_dram_log->data[idx] = (char)c;
			g_dram_log->write_offset = idx + 1;
			g_dram_log->data[idx + 1] = '\0';
		}
	}

	/* 2. Transmit over Qualcomm MSM BLSP UARTDM */
	...
	msm_uart_write(UARTDM_TF, (uint32_t)c);
}
...
msm_uart_init(void)
{
	...
	if (!g_dram_log) {
		...
		if (g_dram_log && g_dram_log->magic != DRAM_LOG_MAGIC) {
			g_dram_log->magic = DRAM_LOG_MAGIC;
			g_dram_log->version = 1;
			g_dram_log->write_offset = 0;
			g_dram_log->boot_count = 1;
			g_dram_log->data[0] = '\0';
		}
	}
}

SECURITY_READ_ONLY_LATE(static struct pe_serial_functions) msm_uart_serial_functions =
{
	.init = msm_uart_init,
	.transmit_ready = msm_uart_transmit_ready,
	.transmit_data = msm_uart_transmit_data,
	.receive_ready = msm_uart_receive_ready,
	.receive_data = msm_uart_receive_data,
	.device = SERIAL_PL011_UART
};

static void
msm_uart_setup(const DeviceTreeNode *const devicetree_node)
{
	if (msm_uart_initted) return;
	msm_uart_initted = true;
	...
	unsigned int reg_size;
	if (SecureDTGetProperty(devicetree_node, "reg", (const void **)&reg, &reg_size) != kSuccess) {
		panic("Unable to find the 'reg' property on the MSM UART devicetree node");
	}
	const vm_offset_t soc_base_phys = pe_arm_get_soc_base_phys();
	msm_uart_base = ml_io_map(soc_base_phys + reg->block_offset, reg->block_size);
	register_serial_functions(&msm_uart_serial_functions);
}
...
#endif // PL011_UART
	{ .compatible = "qcom,msm-uartdm", .setup = msm_uart_setup },
};
...
serial_init(void)
{
	...
	if (!get_serial_device_phandle(&phandle)) {
		const DeviceTreeNode *uart_node;
		if (SecureDTFindNodeWithStringProperty("name", "uart", &uart_node) == kSuccess) {
			msm_uart_setup(uart_node);
			goto found_driver;
		}
