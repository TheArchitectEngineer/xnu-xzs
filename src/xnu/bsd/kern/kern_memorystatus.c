...
memorystatus_init(void)
{
	...
	xzs_early_puts("    [memorystatus_init] enter\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("    [memorystatus_init] os_log_create done\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("    [memorystatus_init] buckets & demotion call done\n");
	xzs_watchdog_pet();
	 ...
	 * Use static, 64-byte aligned buffer for memorystatus_jetsam_snapshot
	 * with capacity = 2 entries to bypass multi-page dynamic kalloc_data stall
	 * prior to root mount.
	 * Audited: All consumers respect memorystatus_jetsam_snapshot_max.
	 * memorystatus_jetsam_snapshot is permanent and never freed by XNU.
	 */
	static uint8_t xzs_memorystatus_jetsam_snapshot_buf[
	    sizeof(memorystatus_jetsam_snapshot_t) +
	    (sizeof(memorystatus_jetsam_snapshot_entry_t) * 2)] __attribute__((aligned(64)));
	static bool memorystatus_jetsam_snapshot_is_static = true;
#pragma unused(memorystatus_jetsam_snapshot_is_static)

	memorystatus_jetsam_snapshot_max = 2;
	memorystatus_jetsam_snapshot_size = sizeof(xzs_memorystatus_jetsam_snapshot_buf);
	memset(xzs_memorystatus_jetsam_snapshot_buf, 0, memorystatus_jetsam_snapshot_size);
	memorystatus_jetsam_snapshot = (memorystatus_jetsam_snapshot_t *)(void *)xzs_memorystatus_jetsam_snapshot_buf;

	extern void xzs_early_puthex64(uint64_t v);

	xzs_early_puts("      [MEMSTAT-JS-STATIC] [XZS-WORKAROUND] static buf ptr=0x");
	xzs_early_puthex64((uint64_t)memorystatus_jetsam_snapshot);
	xzs_early_puts(" max=0x");
	xzs_early_puthex64(memorystatus_jetsam_snapshot_max);
	xzs_early_puts(" size=0x");
	xzs_early_puthex64(memorystatus_jetsam_snapshot_size);
	xzs_early_puts("\n");
	xzs_watchdog_pet();
#else
	...
	xzs_early_puts("      [MEMSTAT-JS-0] size=0x");
	xzs_early_puthex64(memorystatus_jetsam_snapshot_size);
	xzs_early_puts(" max=0x");
	xzs_early_puthex64(memorystatus_jetsam_snapshot_max);
	xzs_early_puts(" hdr=0x");
	xzs_early_puthex64(sizeof(memorystatus_jetsam_snapshot_t));
	xzs_early_puts(" ent=0x");
	xzs_early_puthex64(sizeof(memorystatus_jetsam_snapshot_entry_t));
	xzs_early_puts("\n");

	xzs_early_puts("      [MEMSTAT-JS-1] before kalloc_data\n");
	memorystatus_jetsam_snapshot = kalloc_data(memorystatus_jetsam_snapshot_size, Z_WAITOK | Z_ZERO);
	xzs_early_puts("      [MEMSTAT-JS-2] after kalloc_data ptr=0x");
	xzs_early_puthex64((uint64_t)memorystatus_jetsam_snapshot);
	xzs_early_puts("\n");
	if (!memorystatus_jetsam_snapshot) {
		panic("Could not allocate memorystatus_jetsam_snapshot");
	}
	xzs_early_puts("    [memorystatus_init] jetsam snapshot allocated\n");
	xzs_watchdog_pet();
#endif
	...
	xzs_early_puts("    [memorystatus_init] creating jetsam threads\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("    [memorystatus_init] jetsam threads created\n");
	xzs_watchdog_pet();
	...
	/*
	 * [XZS-WORKAROUND] CONFIG_XZS_BRINGUP:
	 * Defer dynamic allocation of diagnostic jzs_meminfo during bringup.
	 * jzs_meminfo is only used for zprint diagnostics during process kills,
	 * completely unreferenced by VFS core or root device selection.
	 */
	jzs_zone_cnt = 0;
	jzs_names = NULL;
	jzs_info = NULL;
