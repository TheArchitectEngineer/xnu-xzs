...
fsevents_internal_init(void)
{
	...
	xzs_early_puts("      [fsevents_internal_init] enter\n");
	xzs_watchdog_pet();
	 ...
	 * Cap max_kfs_events to 8 entries (instead of default 4096) for bringup.
	 * fsevents is userspace file event notification daemon and is not used
	 * for VFS root mount or block device discovery.
	 */
	max_kfs_events = 8;
#endif

	xzs_early_puts("      [fsevents_internal_init] creating zone\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("      [fsevents_internal_init] filling initially\n");
	xzs_watchdog_pet();

	zone_fill_initially(event_zone, max_kfs_events);

	xzs_early_puts("      [fsevents_internal_init] done\n");
	xzs_watchdog_pet();
}
...
fsevents_init(void)
{
	...
	xzs_early_puts("    [fsevents_init] enter\n");
	xzs_watchdog_pet();

	if (fsevents_installed) {
		xzs_early_puts("    [fsevents_init] already installed\n");
		return;
	}

	fsevents_installed = 1;

	xzs_early_puts("    [fsevents_init] calling cdevsw_add\n");
	xzs_watchdog_pet();
	ret = cdevsw_add(-1, &fsevents_cdevsw);
	xzs_early_puts("    [fsevents_init] cdevsw_add ret=0x");
	xzs_early_puthex64((uint64_t)(uint32_t)ret);
	xzs_early_puts("\n");
	xzs_watchdog_pet();
	if (ret < 0) {
	...
	xzs_early_puts("    [fsevents_init] calling devfs_make_node\n");
	xzs_watchdog_pet();
	devfs_make_node(makedev(ret, 0), DEVFS_CHAR,
	    UID_ROOT, GID_WHEEL, 0644, "fsevents");
	xzs_early_puts("    [fsevents_init] devfs_make_node done\n");
	xzs_watchdog_pet();

	fsevents_internal_init();
	xzs_early_puts("    [fsevents_init] done\n");
	xzs_watchdog_pet();
}
