...
{
	/*
	 * XZS-WORKAROUND: In headless early bootstrap without PrelinkedKernel kexts,
	 * pthread.kext is not loaded. Skipping avoids panic before VFS mountroot.
	 */
	if (!pthread_functions) {
		printf("pthread_init: pthread kernel extension not loaded, skipping\n");
		return;
	}
