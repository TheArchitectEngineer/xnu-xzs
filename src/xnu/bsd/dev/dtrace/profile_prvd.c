...
profile_attach(dev_info_t *devi)
{
	...
	xzs_early_puts("        [profile_attach] ENTER\n");
	xzs_watchdog_pet();
	...
	if (ddi_create_minor_node(devi, "profile", S_IFCHR, 0,
		...
		xzs_early_puts("        [profile_attach] failed!\n");
	...
	xzs_early_puts("        [profile_attach] SUCCESS\n");
	xzs_watchdog_pet();
	return DDI_SUCCESS;
}
...
profile_init( void )
{
	...
	/*
	 * [XZS-WORKAROUND] Defer DTrace Profile provider during Phase D1 bring-up.
	 * Profile is a userspace debug sampling provider with no consumers in VFS or
	 * root filesystem mounting.
	 */
	xzs_early_puts("      [XZS-BOOT] [XZS-WORKAROUND] profile_init DEFERRED\n");
	xzs_watchdog_pet();
	return;
#else
	xzs_early_puts("      [profile] profile_init ENTER\n");
	xzs_watchdog_pet();

	int majdevno = cdevsw_add(PROFILE_MAJOR, &profile_cdevsw);

	if (majdevno < 0) {
		xzs_early_puts("      [profile] cdevsw_add failed!\n");
	...
	xzs_early_puts("      [profile] profile_init DONE\n");
	xzs_watchdog_pet();
#endif
}
#undef PROFILE_MAJOR
