...
fbt_attach(dev_info_t *devi)
{
	...
	xzs_early_puts("        [fbt_attach] ENTER\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("        [fbt_attach] kmem_zalloc probetab...\n");
	xzs_watchdog_pet();
	fbt_probetab =
	    kmem_zalloc(fbt_probetab_size * sizeof(fbt_probe_t *), KM_SLEEP);

	xzs_early_puts("        [fbt_attach] probetab allocated, dtrace_invop_add...\n");
	xzs_watchdog_pet();
	dtrace_invop_add(fbt_invop);

	xzs_early_puts("        [fbt_attach] ddi_create_minor_node + dtrace_register...\n");
	xzs_watchdog_pet();
	if (ddi_create_minor_node(devi, "fbt", S_IFCHR, 0,
		...
		xzs_early_puts("        [fbt_attach] failed!\n");
		fbt_cleanup(devi);
		return DDI_FAILURE;
	}

	xzs_early_puts("        [fbt_attach] SUCCESS\n");
	xzs_watchdog_pet();
	return DDI_SUCCESS;
}
...
fbt_init( void )
{
	...
	/*
	 * [XZS-WORKAROUND] Defer DTrace FBT provider during Phase D1 bring-up.
	 * FBT is a userspace debug tracing provider with no consumers in VFS or
	 * root filesystem mounting. Deferring eliminates allocator / probe-table
	 * memory pressure during early boot to reach the storage boundary cleanly.
	 */
	xzs_early_puts("      [XZS-BOOT] [XZS-WORKAROUND] fbt_init DEFERRED\n");
	xzs_watchdog_pet();
	return;
#else
	xzs_early_puts("      [fbt] fbt_init ENTER\n");
	xzs_watchdog_pet();

	int majdevno = cdevsw_add(FBT_MAJOR, &fbt_cdevsw);

	if (majdevno < 0) {
		xzs_early_puts("      [fbt] cdevsw_add failed!\n");
		printf("fbt_init: failed to allocate a major number!\n");
		return;
	}

	xzs_early_puts("      [fbt] fbt_blacklist_init...\n");
	xzs_watchdog_pet();
	fbt_blacklist_init();

	xzs_early_puts("      [fbt] fbt_attach...\n");
	xzs_watchdog_pet();
	fbt_attach((dev_info_t*)(uintptr_t)majdevno);

	xzs_early_puts("      [fbt] fbt_init DONE\n");
	xzs_watchdog_pet();
#endif
}
#undef FBT_MAJOR
