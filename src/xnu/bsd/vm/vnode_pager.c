	...
	int                     *errorp)
{
	...
	if (upl == (upl_t)NULL) {
		...
		if (vp->v_mount->mnt_vtable->vfc_vfsflags & VFC_VFSVNOP_PAGEINV2) {
			...
			{
				char pibuf[128];
				int pilen = snprintf(pibuf, sizeof(pibuf), "[XZS-T2N3] VNOP_PAGEIN_CALLED: vp=%p upl_offset=0x%x f_offset=0x%llx size=%u\n", (void *)vp, upl_offset, (unsigned long long)f_offset, size);
			...
			if ((error = VNOP_PAGEIN(vp, NULL, upl_offset, (off_t)f_offset,
				...
				char pibuf[64];
				int pilen = snprintf(pibuf, sizeof(pibuf), "[XZS-T2N3] VNOP_PAGEIN_RETURN: error=%d\n", error);
				xzs_bringup_console_write(pibuf, pilen);
	...
	while (last_pg < pages_in_upl) {
		...
		if (last_pg > start_pg) {
			...
			{
				char pibuf[128];
				int pilen = snprintf(pibuf, sizeof(pibuf), "[XZS-T2N3] VNOP_PAGEIN_CALLED: vp=%p upl=%p xoff=%d f_offset=0x%llx size=%d\n", (void *)vp, (void *)upl, xoff, (unsigned long long)f_offset + xoff, xsize);
				xzs_bringup_console_write(pibuf, pilen);
			}
			error = VNOP_PAGEIN(vp, upl, (upl_offset_t) xoff,
			    (off_t)f_offset + xoff,
