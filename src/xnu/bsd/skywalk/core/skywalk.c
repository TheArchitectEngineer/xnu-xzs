...
static bool sk_dump_buf_is_static = false;
#endif
...
#if (SK_LOG || DEVELOPMENT || DEBUG)
#if CONFIG_XZS_BRINGUP
		if (sk_dump_buf != NULL && !sk_dump_buf_is_static) {
#else
		if (sk_dump_buf != NULL) {
#endif
...
skywalk_init(void)
{
	...
	xzs_early_puts("    [skywalk_init] entering SK_LOCK\n");
	xzs_watchdog_pet();

	__sk_inited = 1;

	SK_LOCK();
	xzs_early_puts("    [skywalk_init] skmem_init\n");
	xzs_watchdog_pet();
	skmem_init();
	xzs_early_puts("    [skywalk_init] nexus_init\n");
	xzs_watchdog_pet();
	error = nexus_init();
	if (error == 0) {
		xzs_early_puts("    [skywalk_init] channel_init\n");
		xzs_watchdog_pet();
		error = channel_init();
	}
	if (error != 0) {
	...
	xzs_early_puts("    [skywalk_init] SK_UNLOCK passed\n");
	xzs_watchdog_pet();

	if (error == 0) {
#if (SK_LOG || DEVELOPMENT || DEBUG)
		/* allocate space for sk_dump_buf */
		xzs_early_puts("    [skywalk_init] sk_alloc_data\n");
		xzs_watchdog_pet();
#if CONFIG_XZS_BRINGUP
		/*
		 * [XZS-WORKAROUND] CONFIG_XZS_BRINGUP:
		 * Avoid blocking on zone_expand_locked(kalloc.data.2048) before vm_pageout.
		 * Attempt Z_NOWAIT allocation first. If unavailable, fallback to static
		 * aligned scratch buffer to satisfy VERIFY(sk_dump_buf != NULL) in
		 * skywalk_self_tests() and sk_dump().
		 */
		void *buf = sk_alloc_data(SK_DUMP_BUF_SIZE, Z_NOWAIT, skmem_tag_dump);
		if (buf == NULL) {
			static char s_sk_dump_buf[SK_DUMP_BUF_SIZE] __attribute__((aligned(16)));
			buf = s_sk_dump_buf;
			sk_dump_buf_is_static = true;
			xzs_early_puts("    [skywalk_init] sk_alloc_data: using static scratch buffer\n");
		} else {
			sk_dump_buf_is_static = false;
			xzs_early_puts("    [skywalk_init] sk_alloc_data: allocated dynamically\n");
		}
		sk_dump_buf = buf, sk_dump_buf_size = SK_DUMP_BUF_SIZE;
#else
		...
		xzs_early_puts("    [skywalk_init] netns_init\n");
		xzs_watchdog_pet();
		netns_init();
		xzs_early_puts("    [skywalk_init] protons_init\n");
		xzs_watchdog_pet();
		protons_init();
		xzs_early_puts("    [skywalk_init] flowidns_init\n");
		xzs_watchdog_pet();
		flowidns_init();

#if (DEVELOPMENT || DEBUG)
		xzs_early_puts("    [skywalk_init] skywalk_self_tests\n");
		xzs_watchdog_pet();
		skywalk_self_tests();
		xzs_early_puts("    [skywalk_init] skmem_test_init\n");
		xzs_watchdog_pet();
		skmem_test_init();
		xzs_early_puts("    [skywalk_init] cht_test_init\n");
		xzs_watchdog_pet();
		cht_test_init();
#endif /* DEVELOPMENT || DEBUG */
	}

	xzs_early_puts("    [skywalk_init] EXIT\n");
	xzs_watchdog_pet();
