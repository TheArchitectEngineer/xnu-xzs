...
skmem_init(void)
{
	...
	xzs_early_puts("      [skmem_init] ENTER\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("      [skmem_init] cache line size ready\n");
	xzs_watchdog_pet();

	skmem_cache_pre_init();
	xzs_early_puts("      [skmem_init] cache_pre_init done\n");
	xzs_watchdog_pet();

	skmem_region_init();
	xzs_early_puts("      [skmem_init] region_init done\n");
	xzs_watchdog_pet();

	skmem_cache_init();
	xzs_early_puts("      [skmem_init] cache_init done\n");
	xzs_watchdog_pet();

	pp_init();
	xzs_early_puts("      [skmem_init] pp_init done\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("      [skmem_init] EXIT\n");
	xzs_watchdog_pet();
}
...
#define CONFIG_XZS_BRINGUP 1
	...
	/* [XZS-WORKAROUND] Return canonical Kryo ARM64 cache line size (64 bytes) to avoid cluster topology wait */
...
skmem_sys_region_init(void)
{
	...
	xzs_early_puts("      [skmem_sys_region_init] skmem_region_create\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("      [skmem_sys_region_init] skmem_region_alloc\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("      [skmem_sys_region_init] skmem_sysctl_init\n");
	xzs_watchdog_pet();
	skmem_sysctl_init();
	xzs_early_puts("      [skmem_sys_region_init] done\n");
	xzs_watchdog_pet();
}
