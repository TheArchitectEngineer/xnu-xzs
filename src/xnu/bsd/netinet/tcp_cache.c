...
tcp_cache_init(void)
{
	 ...
	 * Cap TCP cache size to 32 entries during early bring-up to avoid
	 * zalloc_permanent() allocation stall before VM paging/pageout is ready.
	 */
	cache_size = 32;
#else
	if (cache_size < 32) {
	...
	xzs_early_puts("        [tcp_cache_init] enter, cache_size=0x");
	xzs_early_puthex64((uint64_t)cache_size);
	xzs_early_puts("\n");
	xzs_watchdog_pet();

	xzs_early_puts("        [tcp_cache_init] step 1: allocating tcp_cache\n");
	xzs_watchdog_pet();
	tcp_cache = zalloc_permanent(sizeof(struct tcp_cache_head) * cache_size,
	    ZALIGN(struct tcp_cache_head));
	tcp_cache_size = cache_size;

	xzs_early_puts("        [tcp_cache_init] step 2: allocating tcp_heuristics\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("        [tcp_cache_init] step 3: initting mutexes\n");
	xzs_watchdog_pet();
	...
	xzs_early_puts("        [tcp_cache_init] step 4: RandomULong\n");
	xzs_watchdog_pet();
	tcp_cache_hash_seed = RandomULong();
	xzs_early_puts("        [tcp_cache_init] done\n");
	xzs_watchdog_pet();
}
