...
vm_deferred_reclamation_init(void)
{
	...
	xzs_early_puts("    [vm_deferred_reclamation_init] ENTER\n");

	vm_reclaim_log_handle = os_log_create("com.apple.xnu", "vm_reclaim");
	xzs_early_puts("    [vm_deferred_reclamation_init] 1: os_log_create done\n");

	nanoseconds_to_absolutetime((uint64_t)vm_reclaim_sampling_period_ns,
	    &vm_reclaim_sampling_period_abs);
	xzs_early_puts("    [vm_deferred_reclamation_init] 2: absolutetime done\n");

	sched_cond_init(&vm_reclaim_scavenger_cond);
	lck_mtx_gate_init(&reclaim_buffers_lock, &vm_reclaim_gc_gate);
	xzs_early_puts("    [vm_deferred_reclamation_init] 3: gate init done\n");
	...
	xzs_early_puts("    [vm_deferred_reclamation_init] 4: thread start done\n");
	if (kr != KERN_SUCCESS) {
		panic("Unable to create VM reclaim thread, %d", kr);
	}
	xzs_early_puts("    [vm_deferred_reclamation_init] RETURN\n");
}
