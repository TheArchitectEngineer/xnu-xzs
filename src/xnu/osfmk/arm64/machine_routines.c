...
void
ml_cpu_signal(unsigned int cpu_mpidr)
{
	uint64_t aff1 = (cpu_mpidr >> 8) & 0xFFULL;
	uint64_t aff0 = cpu_mpidr & 0x0FULL;
	uint64_t target_list = 1ULL << aff0;
	uint64_t sgi1r = (1ULL << 24) | (aff1 << 16) | target_list; // SGI 1
	__asm__ volatile(
		"dsb ishst\n"
		"msr ICC_SGI1R_EL1, %0\n"
		"isb sy\n"
...
void
ml_cpu_signal_deferred(unsigned int cpu_mpidr)
{
	ml_cpu_signal(cpu_mpidr);
}

void
ml_cpu_signal_retract(unsigned int cpu_mpidr __unused)
{
	/* ARM GICv3 SGIs cannot be retracted once posted */
}
...
machine_startup(__unused boot_args * args)
{
	...
	*(volatile uint32_t *)0x80060014UL = 0x20c; /* XZS_STAGE_XNU_K12 */
	xzs_early_puts("[XNU-XZS] K12: machine_startup ENTERED\n");
	...
	xzs_early_puts("[XNU-XZS] calling machine_conf...\n");
	machine_conf();
	*(volatile uint32_t *)0x80060014UL = 0x20d; /* XZS_STAGE_XNU_K13 */
	xzs_early_puts("[XNU-XZS] K13: machine_conf COMPLETED\n");

	/*
	 * Kick off the kernel bootstrap.
	 */
	xzs_early_puts("[XNU-XZS] calling kernel_bootstrap...\n");
	kernel_bootstrap();
	/* NOTREACHED */
}
...
ml_unsafe_kernel_text_init(void)
{
	...
	/*
	 * [XZS-COMPAT] Qualcomm Snapdragon 820 (MSM8996) / non-iBoot environment:
	 * Apple CTRR/KTRR hardware registers and iBoot DT properties do not exist.
	 * The absence of the Apple iBoot property must not be treated as "uninitialized".
	 * Explicitly configure canonical non-CTRR state:
	 *   _unsafe_kernel_text = false (kernel text is protected / not writable),
	 *   _unsafe_kernel_text_initialized = true.
	 */
	else {
		_unsafe_kernel_text = false;
		_unsafe_kernel_text_initialized = true;
	}
#endif
}
