...
{
	xzs_early_puts("    [VM] arm_vm_init entered\n");
...
#if defined(ARM_LARGE_MEMORY)
	unsigned long physmap_l1_entries = ((real_phys_size + (PTOV_TABLE_SIZE * ARM_TT_TWIG_SIZE) + ARM64_PHYSMAP_SLIDE_RANGE) >> ARM_TT_L1_SHIFT) + 1;
	...
	/* Preserve early debug/UART/mailbox identity mapping in TTBR0 */
	invalid_tte[0] = boot_tte[0];
	xzs_early_puts("    [VM] invalid_tte[0] preserved from boot_tte[0]\n");
	...
	*cpu_l2_tte = (kvtophys(ptpage_vaddr) & ARM_TTE_TABLE_MASK) | ARM_TTE_TYPE_TABLE | ARM_TTE_VALID | ARM_TTE_TABLE_PXN | ARM_TTE_TABLE_XN;
	...
	flush_mmu_tlb();
	kva_active = TRUE;
	xzs_early_puts("    [VM] MMU switched to cpu_tte, kva_active=TRUE\n");
	...
	enable_preemption();
	xzs_early_puts("    [VM] arm_vm_init finished!\n");
}
