...
panic_with_thread_kernel_state(const char *msg, arm_saved_state_t *ss)
{
	...
	xzs_record_fault(1 /* PANIC */, (uint64_t)state->esr, state->pc, state->far);

	xzs_early_puts("\n[PANIC-DETAILS] msg=");
	if (msg) xzs_early_puts(msg);
	xzs_early_puts(" pc=");
	xzs_early_puthex64(state->pc);
	xzs_early_puts(" lr=");
	xzs_early_puthex64(state->lr);
	xzs_early_puts(" esr=");
	xzs_early_puthex64(state->esr);
	xzs_early_puts(" far=");
	xzs_early_puthex64(state->far);
	xzs_early_puts("\n[PANIC-REGS] x0=");
	xzs_early_puthex64(state->x[0]);
	xzs_early_puts(" x1=");
	xzs_early_puthex64(state->x[1]);
	xzs_early_puts(" x2=");
	xzs_early_puthex64(state->x[2]);
	xzs_early_puts(" x3=");
	xzs_early_puthex64(state->x[3]);
	xzs_early_puts("\n[PANIC-REGS] sp=");
	xzs_early_puthex64(state->sp);
	xzs_early_puts(" fp=");
	xzs_early_puthex64(state->fp);
	xzs_early_puts("\n");
...
sleh_synchronous(arm_context_t *context, uint64_t esr, vm_offset_t far, __unused bool did_initiate_panic_lockdown)
{
	...
	if (is_user && xzs_d6m4_probe_armed && thread == xzs_d6m4_target_thread) {
		...
		if (is_user && class == ESR_EC_SVC_64 &&
		    xzs_d6m4_r650_telemetry.svc_trapped == 0) {
			...
			xzs_d6m4_r650_telemetry.post_spsr = spsr;
			xzs_d6m4_r650_telemetry.post_sp_el0 = sp_el0;
			xzs_d6m4_r650_telemetry.post_x0 = ss64->x[0];
			xzs_d6m4_r650_telemetry.post_x16 = ss64->x[16];
...
#define TRACE_SYSCALL 1
	...
	if (trap_no < 0) {
		...
		extern thread_t xzs_d6m4_target_thread;
		extern struct xzs_d6m4_r650_telemetry xzs_d6m4_r650_telemetry;
		if (xzs_d6m4_probe_armed && thread == xzs_d6m4_target_thread && trap_no == 4) {
			xzs_d6m4_r650_telemetry.syscall_classified = 1;
			__asm__ volatile("dmb ish" ::: "memory");
		}
#endif
...
volatile uint32_t g_xzs_timer_irq_count = 0;
volatile uint32_t g_xzs_core_timer_fired[4] = {0};
volatile uint32_t g_xzs_core_ipi_acked[4] = {0};
...
sleh_irq(arm_saved_state_t *state)
{
	...
	// Qualcomm MSM8996 GICv3 Early Dispatcher
	uint64_t iar = 0;
	__asm__ volatile("mrs %0, ICC_IAR1_EL1" : "=r"(iar));
	uint32_t irq_id = (uint32_t)(iar & 0x3ff);

	extern void xzs_early_puts(const char *s);
	extern void xzs_early_puthex64(uint64_t val);

	// Handle GICv3 SGI (Software Generated Interrupt / IPI, INTID 0-15)
	if (irq_id < 16) {
		uint64_t cur_mpidr = 0;
		__asm__ volatile("mrs %0, MPIDR_EL1" : "=r"(cur_mpidr));
		uint32_t core_id = (uint32_t)(cur_mpidr & 0xFF);
		if (core_id == 0 && (cur_mpidr & 0x100)) core_id = 2;
		else if (core_id == 1 && (cur_mpidr & 0x100)) core_id = 3;

		static volatile uint32_t s_ipi_log_count[4] = {0};
		if (core_id < 4 && s_ipi_log_count[core_id] < 2) {
			s_ipi_log_count[core_id]++;
			xzs_early_puts("[IPI] CPU");
			xzs_early_puthex64((uint64_t)core_id);
			xzs_early_puts(" ENTER sgi_id=");
			xzs_early_puthex64((uint64_t)irq_id);
			xzs_early_puts(" ACK & EOI completed\n");
		}

		if (core_id < 4) {
			g_xzs_core_ipi_acked[core_id] = 1;
		}
		if (core_id == 1) {
			extern volatile uint32_t g_xzs_cpu1_ipi_acked;
			g_xzs_cpu1_ipi_acked = 1;
		} else if (core_id == 0) {
			extern volatile uint32_t g_xzs_cpu0_ipi_acked;
			g_xzs_cpu0_ipi_acked = 1;
		} else if (core_id == 2) {
			extern volatile uint32_t g_xzs_cpu2_ipi_acked;
			g_xzs_cpu2_ipi_acked = 1;
		} else if (core_id == 3) {
			extern volatile uint32_t g_xzs_cpu3_ipi_acked;
			g_xzs_cpu3_ipi_acked = 1;
		}

		__asm__ volatile("msr ICC_EOIR1_EL1, %0\nisb" :: "r"(iar));

		if (getCpuDatap() != NULL && (getCpuDatap()->cpu_flags & 0x1000 /* StartedState */)) {
	...
	if (irq_id == 27 || irq_id == 30) {
		...
		if (core_id != 0) {
			if (core_id < 4) {
				g_xzs_core_timer_fired[core_id] = 1;
			}
			extern volatile uint32_t g_xzs_secondary_timer_fired;
			g_xzs_secondary_timer_fired = 1;

			// Mask CNTV timer on secondary core
			uint64_t ctl = 0;
		...
		// Reprogram decrementer while running the 100-timer test; quiesce afterward
		if (g_xzs_timer_irq_count < 105) {
			ml_set_decrementer(96000);
		}

		extern volatile uint32_t g_xzs_preempt_test_active;
		extern thread_t g_thread_c;
		extern void sched_set_kernel_thread_priority(thread_t, int);
		extern void ast_on(ast_t);
		if (g_xzs_preempt_test_active == 1) {
			g_xzs_preempt_test_active = 2; // only trigger once
			xzs_early_puts("[PREEMPT] Timer IRQ in CPU-bound loop! Boosting Thread C to MAXPRI_KERNEL...\n");
			if (g_thread_c) {
				sched_set_kernel_thread_priority(g_thread_c, MAXPRI_KERNEL);
			}
