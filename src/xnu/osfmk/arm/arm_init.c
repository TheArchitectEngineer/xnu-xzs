...
void xzs_panic_hook(const char *str) {
	/* 1. Ensure Fastboot restart reason 0x77665500 in IMEM SRAM */
	*(volatile uint32_t *)0x066bf65cUL = 0x77665500;

	/* 2. Record fault into IMEM SRAM */
	extern void xzs_record_fault(uint32_t fault_type, uint64_t esr, uint64_t elr, uint64_t far);
	xzs_record_fault(1 /* PANIC */, 0, (uint64_t)__builtin_return_address(0), 0);

	volatile uint32_t *stg = (volatile uint32_t *)0x80060014UL;
	if (*stg == 0) {
	...
	xzs_early_puts("Rebooting warm to Fastboot...\n");
	...
	boot_args       *args)
{
	...
	PE_arm_debug_panic_hook = xzs_panic_hook;

	/* Update VBAR_EL1 to KVA so any exception in high virtual address space traps cleanly */
	__asm__ volatile("msr VBAR_EL1, %0\nisb" :: "r"((uint64_t)&LowExceptionVectorBase) : "memory");

	*(volatile uint32_t *)0x80060014UL = 0x206; /* XZS_STAGE_XNU_K6 */
	xzs_early_puts("[XNU-XZS] K6: arm_init entered\n");

	arm_slide_rebase_and_sign_image();

	*(volatile uint32_t *)0x80060014UL = 0x207; /* XZS_STAGE_XNU_K7 */
	xzs_early_puts("[XNU-XZS] K7: image rebased & signed\n");

	/* Checkpoint K7a: Copy boot_args */
	xzs_early_puts("[XNU-XZS] K7a: copying boot args...\n");
	const_boot_args = *args;
	BootArgs = args = &const_boot_args;
	xzs_early_puts("[XNU-XZS] K7a: boot args copied\n");

#if APPLEVIRTUALPLATFORM
	reset_vector_vaddr = (vm_offset_t) &LowResetVectorBase;
#endif /* APPLEVIRTUALPLATFORM */

	/* Checkpoint K7b: Initialize CPU data */
	xzs_early_puts("[XNU-XZS] K7b: calling cpu_data_init...\n");
	cpu_data_init(&BootCpuData);
	xzs_early_puts("[XNU-XZS] K7b: cpu_data_init completed\n");
	...
	/* Checkpoint K7c: Calling PE_init_platform */
	xzs_early_puts("[XNU-XZS] K7c: calling PE_init_platform...\n");
	PE_init_platform(FALSE, args); /* Get platform expert set up */

	*(volatile uint32_t *)0x80060014UL = 0x208; /* XZS_STAGE_XNU_K8 */
	xzs_early_puts("[XNU-XZS] K8: PE_init_platform completed\n");

#if __arm64__
	xzs_early_puts("[XNU-XZS] K8a: configuring timers & apple regs...\n");
	...
	xzs_early_puts("[XNU-XZS] K8a: timers & apple regs configured\n");
	...
	xzs_early_puts("[XNU-XZS] K8b: calling ml_parse_cpu_topology...\n");
	ml_parse_cpu_topology();
	*(volatile uint32_t *)0x80060014UL = 0x208b;
	xzs_early_puts("[XNU-XZS] K8b: ml_parse_cpu_topology completed\n");
	...
	xzs_early_puts("[XNU-XZS] K8b1: starting thread_bootstrap...\n");
	...
	*(volatile uint32_t *)0x80060014UL = 0x208b1;
	xzs_early_puts("[XNU-XZS] K8b1: thread_bootstrap & machine_set_current_thread completed\n");
	...
	rtclock_early_init();
	*(volatile uint32_t *)0x80060014UL = 0x208b2;
	xzs_early_puts("[XNU-XZS] K8b2: cpu_bootstrap & rtclock_early_init completed\n");

	xzs_early_puts("[XNU-XZS] calling kernel_debug_string_early...\n");
	kernel_debug_string_early("kernel_startup_bootstrap");
	xzs_early_puts("[XNU-XZS] calling kernel_startup_bootstrap...\n");
	kernel_startup_bootstrap();
	*(volatile uint32_t *)0x80060014UL = 0x208b3;
	xzs_early_puts("[XNU-XZS] K8b3: kernel_startup_bootstrap completed\n");

	/*
	 * Initialize the timer callout world
	 */
	xzs_early_puts("[XNU-XZS] calling timer_call_init...\n");
	timer_call_init();
	xzs_early_puts("[XNU-XZS] timer_call_init completed\n");

	xzs_early_puts("[XNU-XZS] calling cpu_init...\n");
	cpu_init();
	xzs_early_puts("[XNU-XZS] cpu_init completed\n");

	xzs_early_puts("[XNU-XZS] calling processor_bootstrap...\n");
	processor_bootstrap();
	*(volatile uint32_t *)0x80060014UL = 0x208c;
	xzs_early_puts("[XNU-XZS] K8c: early bootstrap completed\n");
	xzs_early_puts("[XNU-XZS] MILESTONE: Checkpoint K8c verified on MSM8996 real hardware!\n");
	...
	*(volatile uint32_t *)0x80060014UL = 0x208d;
	xzs_early_puts("[XNU-XZS] K8d: dram properties retrieved: base=0x");
	xzs_early_puthex64(gDramBase);
	xzs_early_puts(" size=0x");
	xzs_early_puthex64(gDramSize);
	xzs_early_puts("\n");
	xzs_early_puts("[XNU-XZS] K8d: dram properties retrieved, calling arm_vm_init...\n");
	arm_vm_init(xmaxmem, args);
	*(volatile uint32_t *)0x80060014UL = 0x209; /* XZS_STAGE_XNU_K9 */
	xzs_early_puts("[XNU-XZS] K9: pmap initialized (arm_vm_init completed!)\n");
	xzs_early_puts("[XNU-XZS] MILESTONE: Checkpoint K9 verified on MSM8996 real hardware!\n");
	...
	xzs_early_puts("[XNU-XZS] calling serial_init...\n");
	const unsigned int serial_exists = serial_init();
	xzs_early_puts("[XNU-XZS] serial_init done\n");
	xzs_early_puts("[XNU-XZS] calling kernel_startup_initialize_upto(STARTUP_SUB_KPRINTF)...\n");
	kernel_startup_initialize_upto(STARTUP_SUB_KPRINTF);
	xzs_early_puts("[XNU-XZS] STARTUP_SUB_KPRINTF done, calling kprintf...\n");
	...
	*(volatile uint32_t *)0x80060014UL = 0x20a; /* XZS_STAGE_XNU_K10 */
	xzs_early_puts("[XNU-XZS] K10: serial & console initialized\n");
	xzs_early_puts("[XNU-XZS] MILESTONE: Checkpoint K10 verified on MSM8996 real hardware!\n");
#if __arm64__
#if DEBUG
	dump_kva_space();
#endif
	...
	xzs_early_puts("[XNU-XZS] calling cpu_machine_idle_init...\n");
	cpu_machine_idle_init(TRUE);
	xzs_early_puts("[XNU-XZS] cpu_machine_idle_init completed\n");

	xzs_early_puts("[XNU-XZS] calling PE_init_platform(TRUE)...\n");
	PE_init_platform(TRUE, &BootCpuData);
	xzs_early_puts("[XNU-XZS] PE_init_platform(TRUE) completed\n");

	/* Initialize the debug infrastructure system-wide and on the local core. */
	xzs_early_puts("[XNU-XZS] calling pe_arm_debug_init_early...\n");
	pe_arm_debug_init_early(&BootCpuData);
	xzs_early_puts("[XNU-XZS] pe_arm_debug_init_early completed\n");
	...
	xzs_early_puts("[XNU-XZS] calling ml_map_cpu_pio...\n");
	ml_map_cpu_pio();
	xzs_early_puts("[XNU-XZS] ml_map_cpu_pio completed\n");
	...
	xzs_early_puts("[XNU-XZS] calling cpu_timebase_init...\n");
	cpu_timebase_init(TRUE);
	xzs_early_puts("[XNU-XZS] cpu_timebase_init completed\n");
	...
	xzs_early_puts("[XNU-XZS] calling PE_init_cpu...\n");
	PE_init_cpu();
	xzs_early_puts("[XNU-XZS] PE_init_cpu completed\n");
	...
	xzs_early_puts("[XNU-XZS] calling fiq_context_init...\n");
	fiq_context_init(TRUE);
	xzs_early_puts("[XNU-XZS] fiq_context_init completed\n");
	 ...
	 */
	xzs_early_puts("[XNU-XZS] initializing __stack_chk_guard...\n");
	...
	xzs_early_puts("[XNU-XZS] __stack_chk_guard initialized\n");

	*(volatile uint32_t *)0x80060014UL = 0x20b; /* XZS_STAGE_XNU_K11 */
	xzs_early_puts("[XNU-XZS] K11: arm_init fully completed! Entering machine_startup...\n");
