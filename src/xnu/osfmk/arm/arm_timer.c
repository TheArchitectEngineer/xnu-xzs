	...
	uint64_t                deadline)
{
	...
	if (cpu_data_ptr && cpu_data_ptr->cpu_running) {
		 ...
		 * In early bootstrap / SMP bringup, route safely to master_cpu.
		 */
		if (!ml_is_quiescing()) {
			kprintf("timer_queue_assign: CPU %d !cpu_running, assigning to master_cpu\n",
			    cpu_data_ptr ? cpu_data_ptr->cpu_number : -1);
		}
