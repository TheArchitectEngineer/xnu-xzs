...
{
	read_erandom_generate(buffer, numbytes);
}
...
#define XZS_DRBG_STATE_SIZE (264)

static struct {
	uint8_t drbg_state[XZS_DRBG_STATE_SIZE];
	struct ccdrbg_info drbg_info;
	const struct ccdrbg_nisthmac_custom drbg_custom;
} xzs_prng = {
	.drbg_custom = {
...
static struct cckprng_ctx xzs_opaque_ctx;

static void
xzs_prng_init_with_getentropy(struct cckprng_ctx *ctx __unused,
    unsigned max_ngens __unused,
    size_t seed_nbytes,
    const void *seed,
    size_t nonce_nbytes,
    const void *nonce,
    cckprng_getentropy getentropy __unused,
    void *getentropy_arg __unused)
{
	extern void xzs_early_puts(const char *s);
	extern void xzs_early_puthex64(uint64_t v);
	const char ps[] = "xzs kernel prng";
	int rc;

	xzs_early_puts("  [xzs_prng] init_with_getentropy ENTER\n");

	ccdrbg_factory_nisthmac(&xzs_prng.drbg_info, &xzs_prng.drbg_custom);
	xzs_early_puts("  [xzs_prng] drbg_info.size = ");
	xzs_early_puthex64((uint64_t)xzs_prng.drbg_info.size);
	xzs_early_puts("\n");
	assert3u(xzs_prng.drbg_info.size, <=, sizeof(xzs_prng.drbg_state));

	rc = ccdrbg_init(&xzs_prng.drbg_info,
	    (struct ccdrbg_state *)xzs_prng.drbg_state,
	    seed_nbytes, seed,
	    nonce_nbytes, nonce,
	    sizeof(ps) - 1, ps);
	xzs_early_puts("  [xzs_prng] ccdrbg_init rc = ");
	xzs_early_puthex64((uint64_t)rc);
	xzs_early_puts("\n");
	if (rc != CCDRBG_STATUS_OK) {
		panic("xzs_prng ccdrbg_init failed: %d", rc);
	}
	xzs_early_puts("  [xzs_prng] init_with_getentropy EXIT\n");
}

static void
xzs_prng_initgen(struct cckprng_ctx *ctx __unused, unsigned gen_idx __unused)
{
	extern void xzs_early_puts(const char *s);
	xzs_early_puts("  [xzs_prng] initgen ENTER\n");
	/* Single synchronized DRBG instance handles all CPUs */
}

static void
xzs_prng_refresh(struct cckprng_ctx *ctx __unused)
{
	extern void xzs_early_puts(const char *s);
	xzs_early_puts("  [xzs_prng] refresh ENTER\n");
	/* Optional periodic reseed/refresh from entropy pool */
}

static void
xzs_prng_generate(struct cckprng_ctx *ctx __unused,
    unsigned gen_idx __unused,
    size_t nbytes,
    void *out)
{
	extern void xzs_early_puts(const char *s);
	extern void xzs_early_puthex64(uint64_t v);
	uint8_t *buffer_bytes = (uint8_t *)out;
	int rc;

	while (nbytes > 0) {
		size_t n = MIN(nbytes, PAGE_SIZE);

		rc = ccdrbg_generate(&xzs_prng.drbg_info,
		    (struct ccdrbg_state *)xzs_prng.drbg_state,
		    n, buffer_bytes, 0, NULL);
		if (rc != CCDRBG_STATUS_OK) {
			panic("xzs_prng ccdrbg_generate error %d", rc);
		}
...
static void
xzs_prng_reseed(struct cckprng_ctx *ctx __unused,
    size_t nbytes,
    const void *seed)
{
	extern void xzs_early_puts(const char *s);
	...
	xzs_early_puts("  [xzs_prng] reseed ENTER\n");
	rc = ccdrbg_reseed(&xzs_prng.drbg_info,
	    (struct ccdrbg_state *)xzs_prng.drbg_state,
	    nbytes, seed, 0, NULL);

	if (rc != CCDRBG_STATUS_OK) {
		panic("xzs_prng ccdrbg_reseed error %d", rc);
	}
}

static const struct cckprng_funcs xzs_prng_funcs = {
	.init = NULL,
	.init_with_getentropy = xzs_prng_init_with_getentropy,
	.initgen = xzs_prng_initgen,
	.refresh = xzs_prng_refresh,
	.generate = xzs_prng_generate,
	.reseed = xzs_prng_reseed,
};

void
xzs_prng_bootstrap(void)
{
	/* Ensure early_random_init() / bootseed_init() has derived seed parameters */
	(void)early_random();

	/* Initialize xzs_prng directly (lock-free for early boot) */
	uint64_t nonce = ml_get_timebase();
	xzs_prng_init_with_getentropy(&xzs_opaque_ctx, MAX_CPUS, sizeof(prngseed), prngseed, sizeof(nonce), &nonce, NULL, NULL);

	prng_ctx = &xzs_opaque_ctx;
	prng_funcs = xzs_prng_funcs;
	prng_ready = 1;
}
