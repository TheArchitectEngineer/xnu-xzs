...
extern zone_t xzs_get_thread_call_zone(void);

static inline bool
xzs_is_thread_call_zone(zone_t z)
{
	zone_t tcz = xzs_get_thread_call_zone();
	return (z != NULL && tcz != NULL && z == tcz);
}
#endif
...
#if CONFIG_XZS_BRINGUP
		if (xzs_is_thread_call_zone(zone)) {
			xzs_early_puts("        [ZEXP-SCHEDULE] thread_call_zone scheduled async expand\n");
		}
#endif
...
zone_expand_locked(zone_t z, zalloc_flags_t flags)
{
	...
	for (;;) {
		...
		if (z->z_expander == NULL) {
			z->z_expander = &ze;
#if CONFIG_XZS_BRINGUP
			if (xzs_is_thread_call_zone(z)) {
				xzs_early_puts("        [ZEXP-LOCK-OWNER] assigned z_expander thread=0x");
				xzs_early_puthex64((uint64_t)ze.ze_thread);
				xzs_early_puts("\n");
			}
#endif
			break;
...
#if CONFIG_XZS_BRINGUP
				if (xzs_is_thread_call_zone(z)) {
					xzs_early_puts("        [ZEXP-PG-WAIT] enter VM_PAGE_WAIT\n");
				}
#endif
				VM_PAGE_WAIT();
				continue;
...
#if CONFIG_XZS_BRINGUP
		if (xzs_is_thread_call_zone(z)) {
			xzs_early_puts("        [ZEXP-ZCRAM-DONE] free=");
			xzs_early_puthex64(z->z_elems_free);
			xzs_early_puts("\n");
		}
#endif
...
#if CONFIG_XZS_BRINGUP
		if (xzs_is_thread_call_zone(z)) {
			xzs_early_puts("        [ZEXP-WAKE-INHERITOR] waking expander waiters\n");
		}
#endif
		z->z_expanding_wait = false;
...
#if CONFIG_XZS_BRINGUP
		if (xzs_is_thread_call_zone(z)) {
			xzs_early_puts("        [ZEXP-CALLOUT-MATCH] thread_call_zone refilling\n");
		}
#endif
		zone_lock(z);
...
#if CONFIG_XZS_BRINGUP
			if (xzs_is_thread_call_zone(z)) {
				xzs_early_puts("        [ZEXP-ENTER-EXPAND] calling zone_expand_locked\n");
			}
#endif
			zone_expand_locked(z, Z_WAITOK);
#if CONFIG_XZS_BRINGUP
			if (xzs_is_thread_call_zone(z)) {
				xzs_early_puts("        [ZEXP-EXPAND-DONE] returned from zone_expand_locked\n");
			}
#endif
			/*
