...
zone_t
xzs_get_thread_call_zone(void)
{
	return thread_call_zone->kt_zv.zv_zone;
}
#endif
...
thread_call_initialize(void)
{
	...
	/*
	 * XZS-WORKAROUND: Canonical zone priming for thread_call_zone during
	 * early bring-up to ensure deterministic D1 bootstrap through BSD/NECP.
	 *
	 * Sizing:
	 * At STARTUP_SUB_THREAD_CALL, the initial chunk provides 99 free elements.
	 * Pre-BSD and early BSD demand consumes 96 elements, and Skywalk consumes 3 elements,
	 * leaving exactly 0 elements free upon entering necp_client_init().
	 * NECP client init makes 3 consecutive allocations, needing a reserve of at least 3 elements.
	 *
	 * Because 99 elements are already free at startup, any warmup allocation < 100 is satisfied
	 * entirely from the existing chunk and provisions 0 new elements.
	 * Requesting 105 elements (99 initial + 1 trigger + 5 safety margin matching canonical reserve)
	 * consumes the initial chunk and triggers synchronous zone_expand_locked() on CPU 0 cleanly,
	 * allocating 1 additional physical chunk (+32 elements for 128B elements in 4KB).
	 * Freeing all 105 elements provisions 131 free elements, ensuring >= 29 free elements
	 * remain through NECP with zero expander waits or sleeps.
	 */
	{
		extern void xzs_early_puts(const char *s);
		xzs_early_puts("[TC-PRIME] priming thread_call_zone (count=105)...\n");
		#define TC_PRIME_COUNT 105
		thread_call_t prime_calls[TC_PRIME_COUNT];
		int allocated = 0;
		for (int i = 0; i < TC_PRIME_COUNT; i++) {
			prime_calls[i] = thread_call_allocate_with_options(
			    (thread_call_func_t)thread_call_initialize, NULL,
			    THREAD_CALL_PRIORITY_KERNEL, 0);
			if (prime_calls[i] != NULL) {
				allocated++;
			}
		}
		for (int i = 0; i < allocated; i++) {
			thread_call_free(prime_calls[i]);
		}
		#undef TC_PRIME_COUNT
		zone_raise_reserve(thread_call_zone, 6);
		xzs_early_puts("[TC-PRIME] priming thread_call_zone done\n");
	}
#endif
}
