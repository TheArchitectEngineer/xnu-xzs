...
#pragma clang diagnostic pop
	...
	xzs_early_puts("\n*** XNU KERNEL PANIC: ");
	if (panic_format_str) {
		xzs_early_puts(panic_format_str);
	}
	xzs_early_puts(" caller=");
	xzs_early_puthex64((uint64_t)panic_caller);
	xzs_early_puts(" ***\n");
	xzs_panic_hook(panic_format_str ? panic_format_str : "kernel panic");
	xzs_spin_halt();
...
{
	extern void xzs_early_puts(const char *s);
	xzs_early_puts("    [set_awl_scratch] ENTER\n");
	DTEntry base = NULL;

	if (SecureDTLookupEntry(NULL, "/arm-io/wdt", &base) != kSuccess) {
		xzs_early_puts("    [set_awl_scratch] /arm-io/wdt not found, RETURN\n");
		return;
	}
	...
	xzs_early_puts("    [set_awl_scratch] RETURN\n");
}
